...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.4\textwidth]{diagramas/DomainModel/domain_model.png}}
...
\begin{enumerate}
    \item \textbf{Model:} Contém as classes de dados (\texttt{Produto}, \texttt{Categoria}, \texttt{Cliente}, \texttt{Venda}, \texttt{ItemVenda}, \texttt{Promocao}, \texttt{Ponto}, \texttt{Caixa}, \texttt{Admin}, \texttt{Utilizador}). Estas classes, estereotipadas como \texttt{<<entity>>}, representam as entidades do domínio e não conhecem a interface do utilizador.
    \item \textbf{View:} Responsável por toda a interação com o utilizador via CLI. Inclui as classes \texttt{View} (menus e mensagens), \texttt{CatalogoView}, \texttt{CategoriaView} e \texttt{Utils} (validação de input). As Views apenas recolhem e mostram dados, sem lógica de negócio.
    \item \textbf{Controller:} A classe \texttt{Controller} (estereótipo \texttt{<<controller>>}) é o orquestrador único do sistema. Recebe os Services no construtor, gere o fluxo de navegação entre menus e delega todas as operações de negócio aos Services. Trata exceções com blocos \texttt{try/catch} e comunica resultados através da View genérica.
    \item \textbf{Service:} As classes \texttt{ProdutoService} e \texttt{CategoriaService} (estereótipo \texttt{<<service>>}) contêm a lógica de negócio e validações. Geram IDs automáticos e coordenam a persistência através do Repository.
    \item \textbf{Repository:} A classe \texttt{SupermercadoRepository} (estereótipo \texttt{<<singleton>>}) centraliza toda a persistência de dados em ficheiros CSV. É acedida via \texttt{getInstance()} e expõe referências para os vectores internos.
    \item \textbf{DTO:} Estruturas de transferência de dados (estereótipo \texttt{<<struct>>}) — \texttt{ProdutoDTO}, \texttt{CategoriaDTO}, \texttt{ClienteDTO}, \texttt{CaixaDTO}, \texttt{VendaDTO}, \texttt{ItemVendaDTO}, \texttt{PromocaoDTO} — que transportam dados entre camadas sem expor os Models diretamente às Views.
    \item \textbf{Mappers:} Classes \texttt{ProdutoMapper} e \texttt{CategoriaMapper} (estereótipo \texttt{<<mapper>>}) com métodos estáticos \texttt{toDTO()} que convertem Modelos em DTOs.
    \item \textbf{Exceptions:} Quatro classes de exceção (estereótipo \texttt{<<exception>>}) — \texttt{NoDataException}, \texttt{DuplicatedDataException}, \texttt{InvalidDataException}, \texttt{DataConsistencyException} — que herdam de \texttt{std::exception} e são lançadas pelos Services e capturadas pelo Controller.
\end{enumerate}

O fluxo de uma operação típica é: \textbf{View} recolhe input $\rightarrow$ \textbf{Controller} chama \textbf{Service} $\rightarrow$ \textbf{Service} valida e usa \textbf{Repository} $\rightarrow$ \textbf{Mapper} converte Model $\rightarrow$ DTO $\rightarrow$ \textbf{Controller} passa DTO à \textbf{View} para apresentar.
...
A compilação é gerida pelo \texttt{CMakeLists.txt} (C++17, CMake 3.10+) e o ponto de entrada é o \texttt{main.cpp}.
...
A aplicação utiliza uma interface de linha de comandos (CLI) baseada em menus numéricos. A navegação é feita através da classe \texttt{View}, que apresenta menus e recolhe opções do utilizador.

\subsection{Menu Principal}

Ao iniciar a aplicação, o utilizador escolhe o perfil com que pretende operar:
...
\subsection{Menu ADMIN}

Após selecionar o perfil ADMIN, são apresentadas as seguintes opções:

\begin{verbatim}
--- MENU ADMIN ---
1. Gerir Catalogo
2. Gerir Categorias
3. Gerir Clientes       (em desenvolvimento)
4. Ver Estatisticas     (em desenvolvimento)
0. Sair
\end{verbatim}

\subsubsection{Gerir Catálogo}
...
\subsubsection{Gerir Categorias}
...
\subsection{Menu CAIXA}

Após selecionar um perfil CAIXA, é apresentado o seguinte menu:

\begin{verbatim}
--- MENU CAIXA ---
1. Realizar Venda       (em desenvolvimento)
2. Consultar Preco      (em desenvolvimento)
3. Consultar Pontos     (em desenvolvimento)
0. Sair
\end{verbatim}
...
\section{Conclusão}
Esta segunda iteração focou-se no design e implementação do sistema. Foram aplicados os princípios de arquitetura em camadas (MVC estendido), resultando em 10 classes de domínio (Model), 4 classes de interface (View), 2 Services, 1 Repository Singleton, 7 DTOs, 2 Mappers e 4 classes de exceção. O design foi documentado com 8 diagramas UML detalhando cada camada.

Dos 15 casos de uso identificados na análise de requisitos, 4 foram implementados: Gerir Catálogo (UC01), Gerir Categorias (UC07), Selecionar Perfil (UC14) e Sair do Perfil (UC15). Os diagramas de sequência foram refinados para mostrar o fluxo interno entre as camadas (View $\rightarrow$ Controller $\rightarrow$ Service $\rightarrow$ Repository).
