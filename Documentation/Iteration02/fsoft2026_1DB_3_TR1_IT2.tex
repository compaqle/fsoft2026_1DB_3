...
\begin{enumerate}
    ...
    \item \textbf{Controller:} A classe \texttt{Controller} (com o estereótipo \texttt{<<controller>>}) é a classes que orquestra o funcionamento do sistema. Recebe os Services no construtor, gere o fluxo de navegação entre menus e delega todas as operações de negócio aos Services. Trata exceções com blocos \texttt{try/catch} e comunica resultados através da View genérica.
    ...
    \item \textbf{Exceptions:} Quatro classes de exceção (estereótipo \texttt{<<exception>>}) — \texttt{NoDataException}, \texttt{DuplicatedDataException}, \texttt{InvalidDataException}, \texttt{DataConsistencyException} que herdam de \texttt{std::exception} e são usadas pelos Services e capturadas pelo Controller.
\end{enumerate}

O fluxo de uma operação típica é: \textbf{View} recolhe o input $\rightarrow$ \textbf{Controller} chama \textbf{Service} $\rightarrow$ \textbf{Service} valida e usa \textbf{Repository} $\rightarrow$ \textbf{Mapper} converte Model $\rightarrow$ DTO $\rightarrow$ \textbf{Controller} passa DTO à \textbf{View} para apresentar.
...
\begin{itemize}
    ...
    \item \textbf{Service:} O padrão \textit{Service} é utilizado na classe \texttt{ProdutoService}, onde está a lógica de negócio associada aos produtos (como a atribuição de um ID automático e as verificações que os dados são válidos).
\end{itemize}
