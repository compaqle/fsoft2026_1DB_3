...
\begin{enumerate}
    \item \textbf{Model:} Contém as classes de dados (\texttt{Produto}, \texttt{Cliente}, \texttt{Ponto}, \texttt{Venda}). Estas classes não conhecem a interface do utilizador.
    \item \textbf{View:} Responsável por toda a interação com o utilizador via CLI (\texttt{std::cout} e \texttt{std::cin}).
    \item \textbf{Controller:} Faz a ponte entre as vistas e os dados, aplicando as regras de negócio.
\end{enumerate}

\subsection{Arquitetura de Ficheiros}
O projeto utiliza uma separação rigorosa entre declaração e implementação:
\begin{itemize}
    \item \textbf{\texttt{include/}}: Ficheiros de cabeçalho (\texttt{.h}) com as definições das classes.
    \item \textbf{\texttt{src/}}: Ficheiros de implementação (\texttt{.cpp}) com a lógica dos métodos.
\end{itemize}
