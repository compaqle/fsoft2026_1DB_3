...
\title{Relatório Técnico - Iteração 03\\Sistema de Gestão de Supermercado}
\titlerunning{Relatório Técnico - Iteração 03}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.4\textwidth]{diagramas/DomainModel/domain_model(2).drawio.png}}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{diagramas/design/physical_architecture.drawio.png}}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{diagramas/design/logical_architecture.drawio.png}}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{diagramas/design/code_organization.drawio.png}}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{diagramas/design/model_class_diagram.drawio.png}}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.0\textwidth]{diagramas/design/exceptions_class_diagram.drawio.png}}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.1\textwidth]{diagramas/design/views_class_diagram.drawio.png}}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.0\textwidth]{diagramas/design/controllers_services_class_diagram.drawio.png}}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.1\textwidth]{diagramas/design/dtos_mappers_class_diagram.drawio.png}}
...
\begin{enumerate}
    ...
    \item \textbf{View:} Responsável por toda a interação com o utilizador seja a enviar dados ou a pedir dados. Inclui as classes \texttt{View}, \texttt{CatalogoView}, \texttt{CategoriaView}, \texttt{ClienteView}, \texttt{CaixaView}, \texttt{VendaView}, \texttt{PromocaoView} e \texttt{Utils} (validar o input do utilizador).
    ...
    \item \textbf{Service:} As classes \texttt{ProdutoService}, \texttt{CategoriaService}, \texttt{ClienteService}, \texttt{CaixaService}, \texttt{VendaService}, \texttt{PromocaoService} contêm a lógica de negócio e validações. Geram IDs automáticos e coordenam a persistência através do Repository.
    ...
    \item \textbf{Mappers:} Classes \texttt{ProdutoMapper}, \texttt{CategoriaMapper}, \texttt{ClienteMapper}, \texttt{CaixaMapper}, \texttt{VendaMapper}, \texttt{PromocaoMapper} com métodos estáticos \texttt{toDTO()} que convertem Modelos em DTOs.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{include/view/}} — Headers das classes de interface (\texttt{View.h}, \texttt{CatalogoView.h}, \texttt{CategoriaView.h}, \texttt{ClienteView.h}, \texttt{CaixaView.h}, \texttt{VendaView.h}, \texttt{PromocaoView.h}, \texttt{Utils.h})
    \item \textbf{\texttt{include/controller/}} — Header do orquestrador (\texttt{Controller.h})
    \item \textbf{\texttt{include/services/}} — Headers da lógica de negócio (\texttt{ProdutoService.h}, \texttt{CategoriaService.h}, \texttt{ClienteService.h}, \texttt{CaixaService.h}, \texttt{VendaService.h}, \texttt{PromocaoService.h})
    ...
    \item \textbf{\texttt{include/mappers/}} — Conversores Model-DTO (\texttt{ProdutoMapper.h}, \texttt{CategoriaMapper.h}, \texttt{ClienteMapper.h}, \texttt{CaixaMapper.h}, \texttt{VendaMapper.h}, \texttt{PromocaoMapper.h})
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.1\textwidth]{diagramas/design/ui_navigation.drawio.png}}
...
Na terceira iteração, todos os 15 casos de uso foram implementados:

\begin{itemize}
    \item \textbf{UC01} -- Gerir Catálogo
    \item \textbf{UC02} -- Consultar Estatísticas
    \item \textbf{UC03} -- Gerir Utilizadores (CAIXAs)
    \item \textbf{UC04} -- Repor Stock (integrado no UC01)
    \item \textbf{UC05} -- Gerir CLIENTES
    \item \textbf{UC06} -- Gerir PROMOÇÕES
    \item \textbf{UC07} -- Gerir CATEGORIAS
    \item \textbf{UC08} -- Realizar VENDA
    \item \textbf{UC09} -- Consultar Preço
    \item \textbf{UC10} -- Ver Info Própria (integrado no UC14)
    \item \textbf{UC11} -- Associar CLIENTE (integrado no UC08)
    \item \textbf{UC12} -- Consultar Pontos
    \item \textbf{UC13} -- Consultar RECIBO (integrado no UC08)
    \item \textbf{UC14} -- Selecionar Perfil
    \item \textbf{UC15} -- Sair do Perfil
\end{itemize}

\textbf{Nota:} Os casos de uso UC04, UC10, UC11 e UC13 estão integrados noutros. O UC04 (Repor Stock) faz parte do menu de edição do UC01, o UC10 (Ver Info Própria) está integrado no menu do CAIXA (UC14), e os UC11 (Associar CLIENTE) e UC13 (Consultar RECIBO) estão integrados no fluxo da VENDA (UC08).

Os diagramas de sequência que se seguem mostram o fluxo interno (View $\rightarrow$ Controller $\rightarrow$ Service $\rightarrow$ Repository) para cada caso de uso.
...
\subsection{Restantes Diagramas de Sequência}
...
\subsection{Estrutura dos Testes}

\begin{table}[H]
    \centering
    \begin{tabular}{l c}
        \toprule
        \textbf{Suite} & \textbf{Testes} \\
        \midrule
        ProdutoServiceTest & 6 \\
        CategoriaServiceTest & 6 \\
        ClienteServiceTest & 5 \\
        CaixaServiceTest & 6 \\
        VendaServiceTest & 8 \\
        PromocaoServiceTest & 9 \\
        \midrule
        \textbf{Total} & \textbf{40} \\
        \bottomrule
    \end{tabular}
    \caption{Distribuição dos testes por Service}
\end{table}
...
\begin{itemize}
    \item \textbf{ProdutoService:} Criar produto válido, criar com nome vazio, criar com preço negativo, remover existente, remover inexistente, editar com -1.
    \item \textbf{CategoriaService:} Criar categoria válida, criar com nome vazio, criar com IVA negativo, remover existente, remover inexistente, editar.
    \item \textbf{ClienteService:} Criar cliente válido, criar com NIF duplicado, remover existente, remover inexistente, editar.
    \item \textbf{CaixaService:} Criar caixa válido, criar com nome vazio, remover existente, remover inexistente, editar, editar com -1.
    \item \textbf{VendaService:} Iniciar venda sem cliente, iniciar com cliente, iniciar com cliente inexistente, adicionar item sem venda ativa, adicionar com stock insuficiente, cancelar venda, consultar preço existente, consultar preço inexistente.
    \item \textbf{PromocaoService:} Criar promoção de produto, criar de categoria, percentagem inválida, data inválida, data fim antes de início, produto inexistente, remover existente, remover inexistente, editar.
\end{itemize}
